...
class WrappedProteinChain(ProteinChain):
    ...
    def to_structure_encoder_inputs(
        ...
        chain_index = torch.tensor(np.array([0]*len(coords)), dtype=torch.long, device=device)
    ...
    def from_pdb_multimer(
        ...
        #     bs.filter_amino_acids(atom_array)
        #     & ~atom_array.hetero
        #     & np.isin(atom_array.chain_id, chain_list)
        # ]
        ...
        for i, res in enumerate(bs.residue_iter(atom_array)):
            chain = atom_array[np.isin(atom_array.chain_id, chain_list)]
            assert isinstance(chain, (bs.AtomArray, bs.AtomArrayStack))
