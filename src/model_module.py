...
class ProceedingBaseModel(nn.Module):
    ...
    def inference_feature(self, input_list):
        ...
        input_ids, input_mask, seqs, residue_indices = input_list

        residue_indices = torch.tensor(residue_indices, device=input_ids.device)
        residue_indices = torch.clamp(residue_indices, min=0, max=4999)
        position_embeddings = self.position_embed(residue_indices)
...
class PlModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        split = "training"
        log_metrics = {
            f"{np.split}_{k}": v for k, v in metrics.items()
        }
