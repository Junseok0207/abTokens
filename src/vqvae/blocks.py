...
class VanillaUnifiedTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        """
        if self.use_plain_attn:
            attn_weight, r1 = self.attn(x, attention_mask, chain_id, attn_bias=attn_bias)
            x = x + r1 / self.scaling_factor

        if self.use_geom_attn:
            attn_weight, r2 = self.geom_attn(x, attention_mask, frames, frames_mask, sequence_id, chain_id)
            x = x + r2 / self.scaling_factor

        r3 = self.ffn(x) / self.scaling_factor
        x = x + r3

        return attn_weight, x