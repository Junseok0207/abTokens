...
class VanillaGeometricReasoningOriginalImpl(nn.Module):
    ...
    def forward(self, s, attention_mask, affine, affine_mask, sequence_id, chain_id):
        ...
        # 이걸 안 키고 싶으면 input을 줄 때 그냥 chain_id를 다 같게 줌
        ...
        # affine.rot[..., None].apply(rearrange(vec_rot, "... (h c) -> ... h c", c=3))
        ...
        affine[..., None].apply(rearrange(vec_dist, "... (h c) -> ... h c", c=3)).chunk(2, dim=-2)
