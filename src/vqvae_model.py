...
class VanillaStructureTokenEncoder(nn.Module):
    ...
    """
    def __init__(self, d_model, n_heads, v_heads, n_layers, d_out, n_codes, **kwargs):
        super().__init__()
    ...
    def encode_local_structure(
        ...
        _, z, _ = self.transformer.forward(
            x=z,
...
class VanillaStructureTokenDecoder(nn.Module):
    ...
    def decode(
        ...
        _, x, _ = self.decoder_stack.forward(
            x, attention_mask=attention_mask, affine=None, affine_mask=None, sequence_id=sequence_id, chain_id=chain_id
        ) # [B, L, d_model], [B, L, d_model]
...
class VQVAEModel(nn.Module):
    def __init__(self, model_cfg):
        ...
        self.quantizer_cfg = model_cfg.quantizer
        self.loss_weight = self.quantizer_cfg["loss_weight"]
        self.quantizer = eval(self.quantizer_cfg["quantizer_type"])(**self.quantizer_cfg)
...
class LightningVQFinetuningModel(LightningVQPretrainModel):
    """
    PTL wrapper class for VQ-VAE pre-training
    """
    ...
    def setup(self, stage: str):
        ...
        self.trainer.strategy.config["train_micro_batch_size_per_gpu"] = self.optimizer_cfg.micro_batch_size
        self.model = model_init_fn(self.trainer, self.model_cfg)
        model_states = torch.load(self.model_cfg.pretrained_ckpt_path, map_location=self.device)["module"]
        new_model_states = {}
        for k,v in model_states.items():
            assert k.startswith("model.")
            new_model_states[k[6:]] = v

        if self.model_cfg.encoder.adapter is None:
            # self.model.load_state_dict(new_model_states)
            self.model.load_state_dict(new_model_states, strict=False) # contact prediction head may be different

        else:
            self.model.load_state_dict(new_model_states, strict=False) # some keys may be different
            for param in self.model.parameters():
                param.requires_grad = False

            for param in self.model.encoder.transformer.adapter_blocks.parameters():
                param.requires_grad = True

        if self.model_cfg.freeze_inv_head == True:
            for param in self.model.inverse_folding_head.parameters():
                param.requires_grad = False
...
class LightningVQEvaluationModel(pl.LightningModule):
    def __init__(
        self,
        model_cfg,
        trainer,
    ...
    def _valid_or_test_step(self, batch, batch_idx, split="validation"):
        ...
        # outputs = self.model.forward_and_save(batch["input_list"])
        if self.model_cfg['task'] == 'invfolding':
            outputs = self.model.forward_invfolding_skempi(batch["input_list"])
            # outputs = self.model.forward_invfolding_rabd(batch["input_list"])
        else:
    ...
    def on_validation_epoch_end(self):
        for split in self.all_split_names:
            ...
            if self.model_cfg['task'] == 'invfolding':
                step_outputs = getattr(self, f"{split}_step_outputs")
                # csv_path = f"./SKEMPI_finetuned_PST.csv"
                csv_path = 'debug.csv'
                with open(csv_path, "w", newline="") as f:
                    writer = csv.writer(f)
                    
                    # CSV header 작성 (키 리스트)
                    headers = list(step_outputs[0].keys())
                    writer.writerow(["batch_idx"] + headers)

                    # 각 배치별 row 작성
                    for idx, out in enumerate(step_outputs):
                        row = [idx] + [float(out[k]) for k in headers]
                        writer.writerow(row)
